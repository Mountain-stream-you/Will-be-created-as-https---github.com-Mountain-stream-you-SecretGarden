...
namespace PointsMall.Common
{
    ...
    {
        #region IEnumerable<T> To Json

        public static string ToJson<TSource>(this IEnumerable<TSource> source)
        {
        ...
        /// <returns></returns>
        public static string GetReceiveAddress(string input)
        {
            if (IsJson(input))
            {
                var ReceiveAddress = JsonConvert.DeserializeObject<ReceiveAddress>(input);
                return $"{ReceiveAddress.Province}-{ReceiveAddress.City}-{ReceiveAddress.Region}-{ReceiveAddress.Town} {ReceiveAddress.DetailAddress}";
            }
            return input;
        }
        /// <summary>
        ...
        /// <param name="input"></param>
        /// <returns></returns>
        public static string GetProductAttribute(string input)
        {
            if (IsJson(input))
            {
                var Attribute = JsonConvert.DeserializeObject<List<JsonAttribute>>(input);
                return string.Join('、', Attribute.Select(k => k.value));
            }
            return input;
        }
        ...
        /// </summary>
        public static int GetValueLength(string json, bool breakOnErr, out int errIndex)
        {
            errIndex = 0;
            int len = 0;
            if (!string.IsNullOrEmpty(json))
        ...
        /// </summary>
        public static bool SetCharState(char c, ref CharState cs)
        {
            cs.CheckIsError(c);
            switch (c)
            {
